...
class HatSentryFuturesPosition(models.Model):
    ...
    entry_price = fields.Float(string="Entry Price", digits=(16, 8), readonly=True)
    mark_price = fields.Float(string="Mark Price", digits=(16, 8), readonly=True)
    position_size = fields.Float(string="Position Size", digits=(16, 8), readonly=True)
    notional_value = fields.Monetary(string="Notional Value", currency_field="currency_id", readonly=True)
    margin = fields.Monetary(string="Margin", currency_field="currency_id", readonly=True)
    leverage = fields.Float(string="Leverage", default=1.0, readonly=True)
    margin_mode = fields.Selection(
        [("isolated", "Isolated"), ("cross", "Cross")], string="Margin Mode", default="cross", readonly=True
    )
    liquidation_price = fields.Float(string="Liquidation Price", digits=(16, 8), readonly=True)
    ...
    unrealized_pnl = fields.Monetary(string="Unrealized PnL", currency_field="currency_id", tracking=True, readonly=True)
    unrealized_pnl_pct = fields.Float(string="Unrealized PnL %", digits=(16, 2), readonly=True)
    stop_loss_price = fields.Float(string="Stop Loss Price", digits=(16, 8))
    has_stop_loss = fields.Boolean(string="Has Stop Loss", compute="_compute_has_stop", store=True, readonly=True)
